...
;Variables
  ...
  !define PRODUCT_NAME "Btn-Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/btnproject/btn-electrum"
  !define PRODUCT_PUBLISHER "Btn Team"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\btn-electrum-win-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\btn-electrum-win-${PRODUCT_VERSION}.exe" "" "$INSTDIR\btn-electrum-win-${PRODUCT_VERSION}.exe" 0

  ;Links btn: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\btn" "" "URL:btn Protocol"
  WriteRegStr HKCU "Software\Classes\btn" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\btn" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\btn\shell\open\command" "" "$\"$INSTDIR\btn-electrum-win-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\btn"
