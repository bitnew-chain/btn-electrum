...
from btn_electrum.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from btn_electrum import bitcoin
from btn_electrum.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        import btn_electrum
        try:
            uri = btn_electrum.util.parse_URI(text, self.app.on_pr)
        except:
            self.app.show_info(_("Not a Btn URI"))
            return
    ...
    def do_save(self):
        ...
        from btn_electrum.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Btn address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Btn Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from btn_electrum.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Btn Request"))
