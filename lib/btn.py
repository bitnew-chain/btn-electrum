...
POW_TARGET_TIMESPACE = 64  # bitcoin is 10 * 60
...
mainnet_block_explorers = {
    'explorer.btn.org': ('http://explorer.btn.org', {
        'tx': 'tx',
        'addr': 'address',
        'contract': 'contract'
    })
...
testnet_block_explorers = {
    'explorer.btn.org': ('https://testnet.btn.org/', {
        'tx': 'tx',
        'addr': 'address',
        'contract': 'contract'
    })
...
def is_hash160(addr):
    ...
    if not len(addr) == 40:
        return False
    for char in addr:
        if (char < '0' or char > '9') and (char < 'A'
                                           or char > 'F') and (char < 'a'
                                                               or char > 'f'):
            return False
    return True
...
def msg_magic(message):
    ...
    return b"\x15Btn Signed Message:\n" + length + message
...
def ser_to_point(Aser):
    ...
    if Aser[0] == 0x04:
        return Point(curve, string_to_number(Aser[1:33]),
                     string_to_number(Aser[33:]), _r)
...
def pubkey_from_signature(sig, h):
    ...
    return MyVerifyingKey.from_signature(
        sig[1:], recid, h, curve=SECP256k1), compressed
...
def serialize_xprv(xtype,
    ...
    xprv = xprv_header(xtype) + bytes(
        [depth]) + fingerprint + child_number + c + bytes([0]) + k
...
def serialize_xpub(xtype,
                   ...
                   fingerprint=b'\x00' * 4,
                   child_number=b'\x00' * 4):
    xpub = xpub_header(xtype) + bytes([depth
                                       ]) + fingerprint + child_number + c + cK
...
def btn_addr_to_bitcoin_addr(btn_addr):
    addr_type, hash160 = b58_address_to_hash160(btn_addr)
