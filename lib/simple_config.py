...
SYSTEM_CONFIG_PATH = "/etc/btn_electrum.conf"
...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("btn_electrum directory", path)
        return path
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "btn_electrum.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/btn-electrum.conf."""
    result = {}
...
def read_user_config(path):
    """Parse and store the user config settings in btn-electrum.conf into user_config[]."""
    if not path:
        return {}
